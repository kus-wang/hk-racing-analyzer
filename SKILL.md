...
根据分析结果中的**概率分布**，按以下规则推荐投注方式，并给出**最推荐方式**：

**1. 独赢（Win）**
- 条件：Top1 概率 >= 30%，**或** 赔率 <= 4 倍
- 推荐理由：热门马胜出概率显著高于其他选手
- 风险提示：热门马赔率低，回报有限

**2. 位置（Place）**
- 条件：Top3 中任意一匹概率 >= 18%
- 推荐理由：比独赢门槛低，前三名任一进入即可中奖（通常14匹马出赛时适用）
- 适用场景：热门马优势明显但不确定能否夺冠

**3. 连赢（Quinella）**
- 条件：Top1 + Top2 概率之和 >= 50%，且两者概率差距 <= 20%
- 推荐理由：前两名置信度均高且实力接近
- 推荐组合：第1名马号 + 第2名马号

**4. 位置连赢（Place Quinella / PQ）**
- 条件：Top3 三匹马概率较为分散（最大-最小差值 > 25%）
- 推荐理由：前三名难以排序时，PQ 比三重彩成本低
- 推荐组合：第1+2名、第1+3名、第2+3名三组全包

**5. 三重彩（Trio）**
- 条件：Top3 均有一定概率（每匹 >= 10%），且三匹概率之和 >= 45%
- 推荐理由：前三名置信度高，覆盖面广
- 推荐组合：第1+2+3名三匹马
